...
const replacement = `          {/* DISPATCH & DELIVERY PARTNER ASSIGNMENT PANEL (UNLOCKED WHEN 100% VERIFIED) */}
          <div className={\`bg-white dark:bg-slate-900 rounded-3xl border p-5 sm:p-6 shadow-sm space-y-5 transition-all duration-300 \${isFullyVerified ? 'border-emerald-500/50 shadow-emerald-500/10' : 'border-slate-200 dark:border-slate-800'}\`}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
              ...
              <div>
                {isFullyVerified ? (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 text-[11px] font-bold rounded-lg border border-emerald-200 dark:border-emerald-500/30">
                    <Sparkles size={12} /> Ready to Dispatch
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 text-[11px] font-bold rounded-lg border border-amber-200 dark:border-amber-500/30">
            ...
            <form onSubmit={handleCompleteDispatch} className="flex flex-col xl:flex-row gap-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                  {[
                    ...
                    >
                      ...
                      <span className={\`text-[9px] font-bold uppercase tracking-wider mt-1.5 \${deliveryPartner === mode.id && isFullyVerified ? 'text-indigo-200' : 'text-slate-500'}\`}>
                        {mode.desc}
                      </span>
                    </button>
                  ))}
              ...
              <div className="flex-1 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <input 
                        ...
                        className="w-full px-3 py-2 bg-white dark:bg-slate-900 text-xs font-semibold rounded-lg border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 disabled:opacity-50 transition-all placeholder:font-normal placeholder:text-slate-400"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
                ...
                <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3">
                  <button
                    ...
                    className="w-full sm:w-auto px-4 py-2 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold rounded-xl border border-slate-200 dark:border-slate-700 transition-all cursor-pointer shadow-sm active:scale-[0.98]"
                  >
                    Exit
                  </button>
                  ...
                  {isFullyVerified ? (
                    <button
                      type="submit"
                      className="w-full sm:w-auto px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 text-white rounded-xl text-xs font-black shadow-lg shadow-emerald-500/30 flex items-center justify-center gap-1.5 cursor-pointer transition-all active:scale-[0.98]"
                    >
                      <Sparkles size={14} />
                      <span>Dispatch & Assign</span>
                      <span className="bg-emerald-700/50 px-1.5 py-0.5 rounded text-[10px] ml-1">{deliveryPartner}</span>
                    </button>
                  ) : (
                    <div className="w-full sm:w-auto px-4 py-2 bg-amber-50 dark:bg-amber-500/10 rounded-xl text-[11px] text-amber-700 dark:text-amber-400 font-bold flex items-center justify-center gap-1.5 border border-amber-200 dark:border-amber-500/30">
                      <AlertCircle size={14} />
...
code = code.replace(/\{\/\* DISPATCH & DELIVERY PARTNER ASSIGNMENT PANEL \(UNLOCKED WHEN 100% VERIFIED\) \*\/\}\s*<div className=\{\`bg-white dark:bg-slate-900 rounded-3xl border p-6 shadow-sm space-y-6 transition-all duration-300 \$\{isFullyVerified \? 'border-emerald-500\/50 shadow-emerald-500\/10' : 'border-slate-200 dark:border-slate-800'\}\`\}>[\s\S]*?(?=\s*<\/div>\s*\)\s*:\s*\(\s*\/\* ===+ \*\/\s*\/\* PAGE VIEW B)/, replacement);
