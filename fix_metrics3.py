...
new_grid_content = '            {/* 8 GLOWING OPERATIONAL METRIC CARDS GRID */}\n            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-8 gap-1.5">\n'
...
for m in metrics:
    ...
    new_grid_content += f"""              {{/* Metric {m['id']} */}}
              <div 
                ...
                className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-2 rounded-xl shadow-xs hover:shadow-md hover:border-{m['color']}-400 dark:hover:border-{m['color']}-600 transition-all cursor-pointer group flex flex-col justify-between gap-1"
              >
                <div className="flex items-center gap-1.5">
                  <div className="p-1.5 bg-{m['color']}-500/10 dark:bg-{m['color']}-500/20 text-{m['color']}-600 dark:text-{m['color']}-400 rounded-lg group-hover:scale-110 transition-transform shrink-0">
                    <{m['icon']} size={{14}} />
                  </div>
                  <span className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider">{m['label']}</span>
                </div>
                <div className="flex flex-col gap-0.5 mt-0.5">
                  <span className="text-[11px] text-slate-800 dark:text-slate-200 leading-tight line-clamp-2" title="{info_text}">{info_text}</span>
                </div>
                <div>
