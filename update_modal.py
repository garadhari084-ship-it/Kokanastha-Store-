...
with open('src/components/DeliveryModule.tsx', 'r') as f:
    content = f.read()

# Find the start of the Order Details Modal
start_idx = content.find('{/* Order Details Modal */}')
if start_idx == -1:
    print("Could not find start index")
    exit(1)

# Find the end of the modal (the closing tag of detailOrder &&)
# It's right before {/ * activeOrder ... or before `</>` or `</div>` at the end
# Let's find `<TodayDeliveryModal` which is before it.
...
with open('src/components/DeliveryModule.tsx', 'w') as f:
    f.write(content)
